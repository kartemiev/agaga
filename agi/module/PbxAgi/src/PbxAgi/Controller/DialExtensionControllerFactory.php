<?php
namespace PbxAgi\Controller;

use Zend\ServiceManager\FactoryInterface;
use Zend\ServiceManager\ServiceLocatorInterface;
use PbxAgi\Controller\DialExtensionController;

class DialExtensionControllerFactory implements FactoryInterface {
    
    public function createService(ServiceLocatorInterface $serviceLocator) {
    	$sl = (method_exists($serviceLocator, 'getServiceLocator'))?$serviceLocator->getServiceLocator():$serviceLocator;    	 
    	return new DialExtensionController(
                $sl->get('ExtensionTable'),
                $sl->get('ClientImpl'),
                $sl->get('ExtensionValidator'),
                $sl->get('AppConfig'),
        		$sl->get('PbxAgi\DialDescriptor\DialOptionsDescriptor'),
    			$sl->get('ChannelVarManager'),
    	        $sl->get('CallEntity')
                );
    }
 
}
